...
Assert-Condition ($releaseBuildJob -match "android-actions/setup-android@") "release-build must set up Android SDK tools for producer metadata."
Assert-Condition ($releaseBuildJob -match "sdkmanager" -and $releaseBuildJob -match "build-tools" -and $releaseBuildJob -match "platforms;android-36") "release-build must install Android build tools and the release compile SDK platform for producer metadata."
...
Assert-Condition ($releaseVerifyJob -match "android-actions/setup-android@") "release-verify must set up Android SDK tools for producer metadata source verification."
Assert-Condition ($releaseVerifyJob -match "sdkmanager" -and $releaseVerifyJob -match "build-tools" -and $releaseVerifyJob -match "platforms;android-36") "release-verify must install Android build tools and the release compile SDK platform for producer metadata source verification."
