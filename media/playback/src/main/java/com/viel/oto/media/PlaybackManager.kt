...
class PlaybackManager(
    ...
    fun setBookPlaybackPlan(plan: BookPlaybackPlan, playWhenReady: Boolean = false) {
        scope.launch {
            ...
            com.viel.oto.logger.PlaybackTimingLogger.logSetPlanEntry(
                bookId = plan.bookId,
            ...
            withContext(Dispatchers.Main) {
                ...
                com.viel.oto.logger.PlaybackTimingLogger.logPreApplyCost(
                    bookId = plan.bookId,
    ...
    private fun applyPlaybackPlan(plan: BookPlaybackPlan) {
        ...
        mediaController?.let { controller ->
            ...
            val totalApplyCost = SystemClock.elapsedRealtime() - applyPlanStart
            com.viel.oto.logger.PlaybackTimingLogger.logApplyPlanSkipped(
                bookId = plan.bookId,
                totalMs = totalApplyCost
            )
        }
    ...
    fun skipToNextAvailableTrack(bookId: String, queueIndex: Int) {
        scope.launch {
            ...
            if (next != null) {
                ...
                com.viel.oto.logger.PlaybackFailureLogger.logSelfHealSuccess(nextIndex)
                mediaController?.let { controller ->
