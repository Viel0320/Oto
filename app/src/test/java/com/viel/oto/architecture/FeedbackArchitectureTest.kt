...
class FeedbackArchitectureTest {
    ...
    fun `player bottom bar no longer produces speed or sleep timer feedback`() {
        ...
        assertFalse(
            ...
            source.contains("import com.viel.oto.event.feedback.FeedbackMessages")
        )
    }
    ...
    fun `chapter list no longer constructs feedback copy`() {
        ...
        assertFalse(
            ...
            source.contains("import com.viel.oto.event.feedback.FeedbackMessage")
        )
        assertFalse(
    ...
    fun `abs layer does not import ui packages`() {
        val offenders = productionSources()
            ...
            .filter { file -> file.readText(Charsets.UTF_8).contains("import com.viel.oto.ui.") }
            .map { file -> file.name }
    ...
    fun `media layer does not import the app event sink`() {
        val offenders = productionSources()
            ...
            .filter { file -> file.readText(Charsets.UTF_8).contains("import com.viel.oto.event.AppEventSink") }
            .map { file -> file.name }
