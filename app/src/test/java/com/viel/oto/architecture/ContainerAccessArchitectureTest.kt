...
class ContainerAccessArchitectureTest {
    ...
    private fun File.isCompositionRoot(sourceRoot: File): Boolean {
        val relativePath = relativeTo(sourceRoot).invariantSeparatorsPath
        return relativePath == "OtoApplication.kt" ||
    ...
    companion object {
        private val forbiddenPatterns = listOf(
            ForbiddenPattern(
                name = "OtoApplication.getContainer(...)",
                regex = Regex("""\bOtoApplication\s*\.\s*getContainer\s*\(""")
            ),
            ForbiddenPattern(
                name = "OtoApplication.getProcessContainer(...)",
                regex = Regex("""\bOtoApplication\s*\.\s*getProcessContainer\s*\(""")
            ),
            ForbiddenPattern(
                name = "OtoApplication.getXxxDependencies(...)",
                regex = Regex("""\bOtoApplication\s*\.\s*get\w+Dependencies\s*\(""")
            ),
            ForbiddenPattern(
