...
/**
 * Online Source Cache Policy (Centralizes TTL fallback rules for remote-derived cache rows)
...
object OnlineSourceCachePolicy {
    ...
    /**
     * Timestamp Freshness Check (Returns whether a persisted online snapshot is still inside its TTL window)
    ...
    /**
     * Range Cache TTL Selection (Chooses stricter expiry when no provider version token is present)
