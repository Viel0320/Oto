...
/**
 * Media-core facts that may require application feedback.
 *
...
sealed interface PlaybackDomainEvent {
    /**
     * Playback plan cannot be applied because its source root is inactive.
     *
    ...
    /**
     * HTTP playback was rejected by the user's security preference.
     *
    ...
    /**
     * The service will close after the completed queue grace period.
     *
    ...
    /**
     * A media-session bookmark command persisted the current playback position.
     *
...
/**
 * Hot stream used by media modules to publish playback facts.
 *
...
interface PlaybackDomainEventSink {
    ...
    /**
     * Publishes a domain event without blocking the media thread.
     *
...
/**
 * Observable publication result for media facts.
 *
...
sealed interface PlaybackDomainEventDeliveryResult {
    ...
    /**
     * The fact entered the playback-domain event stream.
     *
    ...
    /**
     * The stream rejected the playback fact.
     *
...
/**
 * Buffered event stream for playback recovery bursts.
 *
